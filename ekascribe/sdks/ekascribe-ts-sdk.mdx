...
Attach context to a session so generation can draw on prior visits and uploaded files. Context is either `past_sessions` (session IDs) or `attachments` (record IDs, each optionally scoped to a `patient_oid`).
...
await ekascribe.sessions.addSessionContext({
  ...
  context: {
    ...
    attachments: [{ id: 'record-id', patient_oid: 'patient-oid' }],
  },
});
```
