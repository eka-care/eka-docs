...
When a session is created with `upload_type: stream`, [Create Session](/ekascribe/api-reference/sessions/create-session) returns a **`wss://` URL** in the `upload_url` field instead of an HTTPS upload URL. Connect to it and stream audio in real time.

<Info>
This is the alternative to [Upload Audio](/ekascribe/api-reference/sessions/upload-audio) (HTTP binary). A session uses **one** path or the other, decided by `upload_type`:
...
<Steps>
  ...
  <Step title="Call End Session">
    Call [End Session](/ekascribe/api-reference/sessions/end-session) (`POST /voice/v1/sessions/{session_id}/end`). For protocol streaming sessions this is the **single, canonical finalize trigger** — closing the socket flushes audio but does **not** start processing on its own.
  </Step>
  <Step title="Poll for results">
    Poll [Get Session](/ekascribe/api-reference/sessions/get-session) at ~1-second intervals until the status is no longer `202`.
  </Step>
</Steps>
