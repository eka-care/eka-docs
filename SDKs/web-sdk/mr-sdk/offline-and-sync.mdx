...
<Note>
  `updatedAt` is **server-owned** and drives delta sync. When a document's `updatedAt` advances,
  the SDK treats its cached file blobs and smart report as stale and re-fetches them on the next
  `downloadFile` / `getSmartReport` — so edits made on other devices show up automatically.
</Note>
