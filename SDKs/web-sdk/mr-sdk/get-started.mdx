...
- **Vital & Lab Trends**: Plot every logged reading of a single parameter across a patient's reports.
- **Cases**: Organise records into cases (folders) — a record can belong to multiple cases.
- **Linked Profiles**: Read records and cases across a primary profile and its linked dependents.
...
| `baseUrl` | `string` | ⚙️ Optional | Override the API base URL. Takes precedence over `environment`. Pass `''` to issue requests against the same origin (host page proxies the calls) |
| `vaultBaseUrl` | `string` | ⚙️ Optional | Override the vault host used by `getParameterTrend` (parameter/vital trends). Takes precedence over `environment`. Pass `''` for same-origin |
