...
After installation, the `ekacare-cli` command is available for quick document processing:
...
| `close()` | Clean up HTTP sessions |

### Context Manager Support

The SDK supports context managers for proper resource management:

```python
from ekacare_sdk import EkaCareSDK

with EkaCareSDK(client_id="your_id", client_secret="your_secret") as sdk:
    result = sdk.process_document("/path/to/report.pdf")
    response = sdk.get_document_result(result['document_id'])
    print(response['data'])
