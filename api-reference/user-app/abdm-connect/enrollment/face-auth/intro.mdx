...
<Steps>
  <Step title="Initialize Face Auth">
    Call the [Init API](/api-reference/user-app/abdm-connect/enrollment/face-auth/face-auth-init) with the user's Aadhaar number to start a face authentication session. This generates a unique transaction ID and a QR code that the user scans with their mobile device.
  </Step>
  ...
  <Step title="Retrieve User Information">
    Once verified, call the [Verify API](/api-reference/user-app/abdm-connect/enrollment/face-auth/face-auth-verify) to fetch the authenticated user's demographic information from Aadhaar. This includes details like name, date of birth, gender, and address.
  </Step>
</Steps>
...
sequenceDiagram
    ...
    Client->>API: POST /face-auth/init (Aadhaar number)
    ...
    Client->>API: POST /face-auth/verify (Transaction ID)
