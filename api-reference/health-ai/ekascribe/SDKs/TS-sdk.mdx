...
const result = await ekascribe.startRecordingV2({
  templates: ['soap'],                 // required: template IDs for output
  sessionMode: 'consultation',         // optional: 'consultation' | 'dictation'
...
const ekascribe = getEkaScribeInstance({
  access_token: token,
  env: 'PROD',
  allianceConfig: { baseUrl: 'https://api.eka.care/voice/api/v2' },
});
...
**Discovery** — `ekascribe.sessions.getDiscoveryDocument()` and `ekascribe.sessions.getDiscoveryConfig()` mirror the Alliance discovery methods.
