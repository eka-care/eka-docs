...
Register and remove event listeners. See [Events](/api-reference/health-ai/medassist/synapse/core/events). `off` needs the same function reference passed to `on`.
...
Start and stop audio recording. See [Voice & audio](/api-reference/health-ai/medassist/synapse/core/voice).
...
`CONNECTED`, `DISCONNECTED`, `PROGRESS_MESSAGE`, `MESSAGE_CHUNK`, `TIPS_MESSAGE`, `END_OF_STREAM`, `ERROR`, `TOOL_CALL`, `AUDIO_TRANSCRIPT`, `TOOL_START`, `TOOL_END`. See [Events](/api-reference/health-ai/medassist/synapse/core/events#event-reference).
