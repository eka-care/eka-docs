...
The core SDK is event-driven. After [starting a session](/api-reference/health-ai/medassist/synapse/core/quickstart#the-lifecycle), subscribe to events to receive streamed message chunks, tips, tool calls, and lifecycle signals. Drive your UI off these events.
...
| `ERROR` | A stream or server error. See [error codes](/api-reference/health-ai/medassist/synapse/core/api-reference#synapse-realtime-error-codes). |
| `TOOL_CALL` | The agent invokes a tool / asks for input. See [Tool calls](/api-reference/health-ai/medassist/synapse/core/tool-calls). |
| `TOOL_START` | A tool begins executing. |
| `TOOL_END` | A tool finishes executing. |
| `AUDIO_TRANSCRIPT` | A transcript of recorded audio. See [Voice & audio](/api-reference/health-ai/medassist/synapse/core/voice). |
