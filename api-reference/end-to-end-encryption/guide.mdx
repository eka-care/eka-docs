...
payload = {"aadhaar_number": "123456789012", "b": 1}
...
def decrypt_jwe(jwe_dict: dict, key_str: str) -> dict:
    key = jwk.JWK(kty="oct", k=base64.urlsafe_b64encode((key_str*2).encode()).rstrip(b"=").decode())
    token = jwe.JWE()
...
print("Decrypted payload:", decrypt_jwe(encrypted_jwe, "5821234562344567"))
```
