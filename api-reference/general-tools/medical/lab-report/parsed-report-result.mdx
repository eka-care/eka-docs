...
---
Use the `/mr/api/v1/docs/{document_id}/result` endpoint along with the document_id to fetch results. This endpoint returns a JSON response that includes a detailed and structured parsed report.
...
- **Query Parameters**
    - **document_id(required)**
        A unique identifier for the uploaded medical record.
