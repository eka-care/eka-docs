...
Most of the widget's look and behavior comes from the agent config you set in [console.eka.care](https://console.eka.care). When you need to override something per-page, you can do it with HTML attributes on the `<eka-medassist-widget>` element — or programmatically with [`EkaMedAssist.init()`](/ai-tools/synapse/embed/javascript-api).
