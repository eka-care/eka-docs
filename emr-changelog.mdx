...
<Update
  ...
  rss={{
    ...
    description: "• Reverted the experimental new queue dashboard (queue-ui-v2), add-patient flow, global-search overlay, drawer action strip, and inline receipt back to the previous queue implementation\n\n• Reverted Mixpanel tracking and default-open behaviour on the receipt panel\n\n• Reverted the full years-and-months patient age label on Templar receipts back to the prior numeric-months handling — callers must pass `ageInM?: number` again\n\nRead more: https://developer.eka.care/emr-changelog"
  }}
>
