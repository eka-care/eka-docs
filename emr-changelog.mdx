...
- **[EMR] ABDM Consent Management Web SDK** — A new embeddable web widget for ABDM health data consent management is now documented. The widget handles the full consent lifecycle — requesting consent with configurable date ranges, record types, and purposes; tracking consent status (granted, pending, denied, revoked, expired); and redirecting to your records viewer when records arrive. Integrators embed the widget via iframe with token-based authentication. [View docs](/SDKs/web-sdk/consent-management/get-started)
