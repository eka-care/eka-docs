...
<Update
  ...
  rss={{
    ...
    description: "• Flip through a patient's medical records with new Previous and Next buttons inside the record viewer — no need to close and reopen one record at a time\n\n• Shareable links to a specific medical record now open straight to that document\n\nRead more: https://developer.eka.care/weeklyupdates"
  }}
>
...
#### Updates
- **Step through records without closing the viewer** — Opening a medical record now shows Previous and Next buttons at the bottom, plus a counter (like `3/12`), so you can move through a patient's records one after another without going back to the list each time.
- **Open a specific record straight from a link** — Shared or bookmarked links that point to a particular medical record now open that record automatically once the patient's records have loaded.
