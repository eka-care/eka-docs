...
<Update
  ...
  rss={{
    ...
    description: "• Aakaar design-system MCP server runtime fix — generated component metadata is now resolved relative to the bundled module location so the server starts cleanly under stdio and streamable-HTTP transports\n\nRead more: https://developer.eka.care/changelog"
  }}
>

## Fixed

- **[Platform] Aakaar design-system MCP server startup** — The Aakaar design-system MCP server (`@eka/aakaar` MCP) now resolves its generated component metadata bundle relative to the compiled module path instead of the process working directory, so the server starts reliably under both stdio and streamable-HTTP transports. Tool registration for component, icon, token, utility-class, and integration helpers is unaffected.
