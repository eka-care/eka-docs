...
<Update
  ...
  rss={{
    ...
    description: "• Prescription pad extends property groups to examination findings and procedures — structured Since/Note for findings and Date/Note for procedures\n\n• `GET /v1/sym-and-diag/properties` adds optional `procedures`, `resource`, and `category` query parameters for context-scoped property lookup\n\n• ExaminationEntity and Procedure types now carry a `properties` map; examination entities also expose a `source` discriminator (symptom | diagnosis)\n\n• Appointment communication service now propagates JWT identity (cid, idp) across async boundaries via AsyncLocalStorage middleware, tightening auditability of outbound messenger payloads\n\nRead more: https://developer.eka.care/changelog"
  }}
>
