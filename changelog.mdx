...
- **[Platform] Appointment booking service → Messenger webhook identity propagation** — Internal Messenger webhook calls now forward the caller's identity provider and client ID from the request's JWT context via a `jwt-payload` header, falling back to `internal` when no JWT is present. Improves downstream auditability and routing on Messenger without changing the webhook contract for external callers.
