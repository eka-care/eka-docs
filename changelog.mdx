...
- **[Platform] ABDM ECDH Python release pipeline** — The Python release workflow now writes the tagged version directly into `pyproject.toml` instead of relying on a tooling subcommand that was failing during publish, so new Python package versions ship reliably from tag pushes.
