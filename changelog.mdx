...
- **[EMR] Patient profile picture chooser race on the doctor app** — The camera and gallery options in the profile picture upload sheet previously launched their intents from `invokeOnCompletion` on the sheet-hide coroutine, which could fire out of order or be skipped if the coroutine was cancelled, occasionally leaving the chooser stuck open or failing to launch on the first tap. Both options now run the launch inline after `sheetState.hide()` in the same coroutine, so the camera capture and gallery picker fire deterministically on every tap.
