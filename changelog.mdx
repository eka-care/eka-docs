...
<Update
  ...
  rss={{
    ...
    description: "• ABDM ECDH primitives now available as a Java package for partner integrations\n\n• Automated CI and tag-driven release pipelines added for the Go, Java, and Python ABDM ECDH packages\n\nRead more: https://developer.eka.care/changelog"
  }}
>

## Added

- **[Platform] ABDM ECDH Java package** — The ABDM ECDH key-exchange primitives used for secure consent and data-fetch flows are now distributable as a Java package alongside the existing Go and Python builds, letting JVM-based partners perform ECDH-on-Curve25519 (Weierstrass form), HKDF-SHA256 key derivation, and AES-256-GCM encryption without re-implementing the protocol. Requires Java 17+ and ships via JitPack. [View docs](/api-reference/user-app/abdm-connect/care-contexts/ecdh-encryption)

## Hardened

- **[Platform] ABDM ECDH CI and release pipelines** — Added per-language continuous integration workflows that build and test the Go, Java, and Python ABDM ECDH packages on every push and pull request, plus tag-driven release pipelines that publish Go modules, Maven artifacts via JitPack, and Python wheels to PyPI from signed version tags. Gives partner integrators a verifiable, reproducible build trail for the cryptographic primitives used in ABDM consent and data-fetch flows.
