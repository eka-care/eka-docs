...
- **[EMR] Today's date rejected on LMP-EDD and US-EDD calculators** — The validation rules on the prescription pad's Last Menstrual Period EDD and Ultrasound EDD calculators compared `LMP < now` and `USdate < now`, which rejected today's date with a "cannot be in the future" error. Both conditions now use `<=`, so clinicians can record an LMP or ultrasound dated today without hitting a false-positive validation block.
