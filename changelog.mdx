...
<Update
  ...
  rss={{
    ...
    description: "• Aakaar design system tokens module no longer pulls in antd, unblocking the MCP server build for consumers that don't ship antd\n\nRead more: https://developer.eka.care/changelog"
  }}
>

## Fixed

- **[Platform] Aakaar `tokens` no longer imports antd** — The `@eka/aakaar/tokens` entry point previously re-exported antd symbols, forcing every consumer (including the Aakaar MCP server) to install `antd` even when they only needed design tokens. `tokens.ts` now owns the `ColorScheme` type and exposes only antd-free values; antd-specific helpers (`antdTheme`, `getAntdTheme`) have been moved to a dedicated `tokens-antd` entry point that re-exports `ColorScheme` for backwards compatibility. Internal call sites were updated to import antd helpers directly from `tokens-antd`, so the MCP build no longer requires antd as a transitive dependency.
