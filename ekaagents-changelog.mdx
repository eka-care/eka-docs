...
<Update
  ...
  rss={{
    ...
    description: "• Echo Agent Kit ships an audio transcription module with Gemini and Eka Care ASR providers behind a unified async interface\n\n• Released as v0.1.34\n\nRead more: https://developer.eka.care/ekaagents-changelog"
  }}
>

## Added

- **[EkaAgents] Audio transcription module in Echo Agent Kit** — Echo Agent Kit now ships an `echo.audio.transcription` module with a `BaseTranscriber` interface and two pluggable providers: `GeminiTranscriber` (backed by `google-genai`, supports verbatim or prompted transcription against the Gemini 2.0/2.5 model family) and `EkaCareTranscriber` (HTTP ASR provider over the Eka Care voice ASR endpoint, supports `en-IN`, `en-US`, and `hi`). A unified `TranscriberConfig` covers provider selection, model, language, temperature, max output tokens, and request timeout, with `ECHO_DEFAULT_TRANSCRIBER_PROVIDER` and `ECHO_DEFAULT_TRANSCRIBER_MODEL` environment variable defaults. `get_transcriber(...)` and `generate_transcriber_config(...)` factories return an async-callable transcriber that accepts raw bytes, a Files API URI, or a public URL, and returns a structured `TranscriptionResponse` with text, detected language, duration, token usage, and provider-specific details. Eka Care provider is opt-in via the new `echo[ekacare]` extra.

## Improved

- **[EkaAgents] Echo Agent Kit v0.1.34** — Echo Agent Kit released as v0.1.34 with the new audio transcription module bundled in. The Gemini integration error message also now correctly points to `google-genai` (replacing the legacy `google-generativeai` reference). Existing agents that don't import the audio module are unaffected.
