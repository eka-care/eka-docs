...
<Update
  ...
  rss={{
    title: "EkaAgents — September 12, 2026",
    description: "• Agents can now answer from a Weaviate-backed knowledge base, and stay quiet when the corpus does not cover the question\n\n• Past conversations now show a short title instead of a timestamp\n\n• The chat widget can ask for consent once, up front, and remember it\n\n• Faster first paint for the chat widget\n\nRead more: https://developer.eka.care/ekaagents-changelog"
  }}
>

## Added

- **[EkaAgents] Agents can answer from a Weaviate knowledge base** — A knowledge base can now be backed by Weaviate as well as AWS Bedrock. Retrieval is hybrid — it matches on meaning and on wording — and each knowledge base can set how confident a match has to be before the agent uses it. Below that line the agent says the material does not cover the question instead of quoting a weak passage, which is usually the right answer for clinical guidelines. A corpus owned by one workspace can also be shared with another workspace's agent.
- **[EkaAgents] Past conversations show a title** — Every conversation now gets a short title generated from what was discussed, and the sessions list returns it along with the timezone the conversation started in. Session pickers can show "Diabetes medication review" instead of a date and time.
- **[EkaAgents] Ask for consent once, up front** — The chat widget can be configured to show a consent form before the first message and remember that the person accepted, so they are asked once rather than at the start of every conversation.
- **[EkaAgents] CRM leads from moolchand agents** — Agents on the moolchand workspace can now capture a patient as a CRM lead during a conversation, the same way Apollo and RFH agents already could.

## Improved

- **[EkaAgents] The chat widget opens faster** — The widget now applies its theme before it first draws, loads less code up front, and lets the host page warm up authentication in advance. Hosts can also pre-load the agent's configuration before the widget itself starts.
- **[EkaAgents] Fewer repeated nudge calls** — Nudges are fetched once a session is ready and cached, so refreshing the page no longer re-runs them.

## Fixed

- **[EkaAgents] Sending a file no longer breaks the reply** — Sending a file could end the response stream early before the agent had answered.
- **[EkaAgents] A failed conversation list no longer looks empty** — When listing past conversations failed, the result was indistinguishable from having none. The failure is now reported.
- **[EkaAgents] Weak knowledge-base matches rank correctly** — A poorly-matching passage could score below zero and disturb the ordering of results.
